// txt파일에서 메뉴 정보 읽어오기
const files = ['menu1_single.txt', 'menu1_set.txt'];
...
function makeUlLi(menus, divName) {
  const category = divName === ".ul_single" ? "버거__menu1_single" : "버거__menu1_set";
  ...
  menus.forEach(menu => {
    ...
    li.innerHTML = `<a href=detail.html?category=${category}&banner_text=${url}&menuNo=${menu.no}>
                      <div class="thum">
                        <img src="./menu_images/menu1/${menu.img}" alt="${menu.name} 이미지">
                      </div>
                      <div class="menu_name">
                        <p>${menu.name}</p>
                        <p>${menu.eng_name}</p>
