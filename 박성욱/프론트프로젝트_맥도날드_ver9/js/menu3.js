// txt파일에서 메뉴 정보 읽어오기
const files = ['menu3_single.txt', 'menu3_set.txt'];
async function readFileText(fileList) {
  for(const file of fileList) {
    try {
      ...
      if(file === 'menu3_single.txt') {
        printCnt(menus, ".cnt_single");
        makeUlLi(menus, ".ul_single");
        stylingLi(".ul_single");
      }
      else {
...
function makeUlLi(menus, divName) {
  const category = divName === ".ul_single" ? "맥모닝__menu3_single" : "맥모닝__menu3_set";
  ...
  menus.forEach(menu => {
    ...
    li.innerHTML = `<a href=detail.html?category=${category}&banner_text=${url}&menuNo=${menu.no}>
                      <div class="thum">
                        <img src="./menu_images/menu3/${menu.img}" alt="${menu.name} 이미지">
                      </div>
                      <div class="menu_name">
                        <p>${menu.name}</p>
                        <p>${menu.eng_name}</p>
